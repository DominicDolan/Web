...
1. Copy `packages/lins/style/studio/` to `packages/lins/style/<your-theme>/`.
2. Rename `.studio` in `theme.css` (and every other file's `@scope`) to your public root theme class, e.g. `.<yourTheme>`.
3. Rename `.notStudio` in every file to your opt-out class, e.g. `.not<YourTheme>`.
