...
describe("parseLinsStylesheet", () => {
    test("round-trips the root theme stylesheet generated by createThemeStylesheet", () => {
        ...
        expect(parsed.definition).toMatchObject({
            ...
            icons: {
                ...
                fontFaceCss: parserFixtureTheme.icons?.fontFaceCss,
        ...
        expect(parsed.definition.colorThemes).toMatchObject([
            {
                id: "light",
                className: "light",
                colorScheme: "light",
                ...
                colors: {
                    primary: ["oklch(55% 0.22 260)", "white"],
                    accent: ["oklch(68% 0.16 175)", "black"],
                    surface: "oklch(99% 0.01 260)",
                    customBrand: ["rebeccapurple", "white"],
                },
                tokens: parserFixtureTheme.colorThemes[0]!.tokens,
            },
            {
                id: "dark",
                className: "dark",
                ...
                tokens: parserFixtureTheme.colorThemes[1]!.tokens,
            },
        ]);
        expect(parsed.definition.appDefaults).toEqual(expect.arrayContaining(parserFixtureTheme.appDefaults!.map((rule) => expect.objectContaining(rule))));
    });

    test("round-trips an element stylesheet generated by createStylesheet", () => {
        ...
        expect(parsed.definition.categories).toMatchObject({
            ...
            "chip-token": {
                stylesheetId: "button",
                selectors: parserFixtureTheme.categories?.["custom-chip"]?.selectors,
...
describe("parseLinsStylesheet warnings", () => {
    describe("structural at-rule / layer / scope warnings (§6.2, §6.3, §8.3)", () => {
        ...
        test("emits unknown-scope when the @scope wrapper is missing entirely (§8.3)", () => {
    ...
    describe("malformed / unsupported content (§6.4)", () => {
        test("emits malformed-rule for unparseable CSS and continues parsing subsequent rules", () => {
            const css = `button {\n    background: red;\n\n${warningFixtureTheme.createStylesheet("button")}`;
    ...
    describe("theme-root and color-role recovery (§5, §4.5, §8.5)", () => {
        test("emits stylesheet-list-mismatch when @import lines disagree with theme.stylesheets", () => {
        ...
        test("emits missing-color-scheme-declaration and defaults colorScheme to light when it's absent", () => {
        ...
        test("emits ambiguous-theme-root-declaration for unclassifiable content in the theme root rule (§5.1)", () => {
        ...
        test("emits unrecognized-theme-root-declaration for content that can't be attributed to icons or typography", () => {
        ...
        test("requires no warning for zero colorThemes entries, still recovering color-role aliases (§8.5)", () => {
        ...
        test("concatenates multiple @font-face blocks into a single icons.fontFaceCss string (§8.5)", () => {
        ...
        test("requires no warning when optOutClassName follows the not{PascalId} convention with no theme metadata", () => {
            const css = warningFixtureTheme.createStylesheet("button");
    ...
    describe("cosmetic differences that must never warn (§8.6)", () => {
        test("does not warn for quote-style or whitespace-only selector differences", () => {
            const css = warningFixtureTheme.createStylesheet("button").replace(
                "input[type=button]",
                "input[type='button']",
        ...
        test("does not warn for CRLF line endings or a missing trailing semicolon", () => {
            const css = warningFixtureTheme.createStylesheet("button")
                .replace(/\n/g, "\r\n")
