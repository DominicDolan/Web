...
export async function handleServerFunctionRequest(
  ...
  try {
    ...
    return json<ServerFunctionFailure>({ ok: false, error: 'Invalid JSON payload.' }, { status: 400 })
  }

  if (typeof payload.id !== 'string' || !Array.isArray(payload.args)) {
    return json<ServerFunctionFailure>({ ok: false, error: 'Invalid server function payload.' }, { status: 400 })
  }
  ...
  try {
    ...
    return json<ServerFunctionFailure>(
      { ok: false, error: error instanceof Error ? error.message : 'Unknown server function error.' },
      { status: 500, headers: event.responseHeaders },
    )
  }
}
