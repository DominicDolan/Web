...
export interface SolidServerFunctionsPluginOptions {
  ...
  /** Build a Cloudflare Worker when `vite build --mode cloudflare` is used. */
...
export function solidServerFunctions(options: SolidServerFunctionsPluginOptions): Plugin {
  ...
  return {
    ...
    enforce: 'pre',
    config(_, env) {
      const cloudflare = env.command === 'build' && env.mode === (options.cloudflareMode ?? 'cloudflare')
      const serverConditions = cloudflare ? ['cloudflare'] : undefined

      return {
        build: {
          outDir: cloudflare ? '.output/public' : 'dist/client',
        },
        environments: {
          server: {
            consumer: 'server',
            keepProcessEnv: !cloudflare,
            resolve: cloudflare
              ? { conditions: serverConditions, externalConditions: serverConditions }
              : undefined,
            dev: {
            ...
            build: {
              ssr: true,
              target: cloudflare ? 'esnext' : 'node18',
              outDir: cloudflare ? '.output/server' : 'dist/server',
              emptyOutDir: false,
              minify: false,
              rollupOptions: {
